...
## 2026-06-25 Night (04:33 UTC) — QariAI 5-Month Stagnation + `Nuwaisir/Quran_speech_recognizer` + Tarteel Stall Day 8 (Round 41)
*Full report: [research/2026-06-25/night-0433.md](2026-06-25/night-0433.md)*

### Insight 1 — QariAI Stagnant for 5 Months (v1.1.6, Feb 1, 2026): Nearest Phoneme-Feedback Competitor Has Stopped Shipping — Window Opens for Sakina
`qariai.app` (QariAI, iOS `id6757345171`, Android `app.qari.ai`) — identified in Round 27 as "the near-term benchmark, already shipping real-time phoneme-level Makharij correction on both stores." Latest confirmed version: **v1.1.6 (February 1, 2026)** — no update in **~147 days** (Feb → Jun 25). User reviews remain positive through April 2026, so the app functions, but no new features have shipped. **Strategic signals:** (1) AI-only Tajweed apps apparently hit a development ceiling — QariAI can't iterate fast enough to close the gap to human teacher quality. (2) TajweedMate v2.0.27 (June 16) is iterating more actively; QariAI is falling behind even its direct peers. (3) No public roadmap or blog update found. **Pitch line:** *"QariAI ships AI Tajweed feedback — but hasn't shipped an update in 5 months, because AI-only Quran apps hit a ceiling. Sakina pairs AI with live teachers who never plateau."* **Sakina action:** Monitor QariAI for any comeback release; if still at v1.1.6 by August 2026, include the stagnation stat in pitch materials as evidence that AI-only Quran apps are commoditizing/stalling while the live-teacher model has no ceiling.

### Insight 2 — `Nuwaisir/Quran_speech_recognizer` (HuggingFace): Uncatalogued Verse-Positioning Model for Juzz 30 — Old Architecture but Pedagogically Targeted Scope
`huggingface.co/Nuwaisir/Quran_speech_recognizer` — fine-tuned `elgeish/wav2vec2-large-xlsr-53-arabic` on the Quran ASR Challenge dataset. Functionality: listens to recitation → identifies position via edit-distance matching → navigates to the correct ayah. **Coverage restricted to Juzz 30 (Surah 78–114, the last 37 surahs)** — the section 95%+ of beginner learners start with. Architecture is 2020-era XLSR-53 (predates Whisper, wav2vec2-large-v2, Conformer) — likely lower WER than modern models but intentionally narrow domain. **Why it matters despite old architecture:** (1) Domain narrowing (Juzz 30 only) may yield better real-learner WER in its scope than broader models — benchmark needed. (2) The verse-positioning-by-edit-distance approach on the 30th Juzz is a simpler complement to `yayaiu6`'s full-Quran Needleman-Wunsch tracker — usable as a lightweight fallback. **Critical unknowns:** License (check before deployment); PER/WER not published; training data size unknown. **Sakina actions: (1) Check license on HuggingFace model card; (2) Run 10 Juzz 30 clips from `Buraaq/quran-audio-text-dataset`; (3) If WER < 15% + permissive license → use as beginner verse-lock alongside `yayaiu6`.**

### Insight 3 — Tarteel Stall Extends to Day 8 + sherpa-onnx Cohere Transcribe Dart API Already Live (Round 38 Correction)
**Tarteel:** v5.78.2 (June 19, 2026) — **Day 8 of stall**, no v5.79+ on any tracker. Sakina ship window fully intact. **sherpa-onnx Cohere Transcribe CORRECTION:** Round 38 stated "NOT yet in Flutter package v1.13.3; monitor for v1.14.x." This was incorrect. Dart API for Cohere Transcribe was added via **PR #3464 in v1.12.35** and is fully present in current v1.13.3. The INT8 model `sherpa-onnx-cohere-transcribe-14-lang-int8-2026-04-01` (April 2026, MENA/Arabic confirmed) is usable today via Flutter. One file swap in sherpa-onnx's `flutter-examples/streaming_asr/` → on-device Cohere Arabic streaming ASR, zero API cost, fully offline. This resolves the Round 38 action item. **All 4 monitors unchanged:** `uzair0/quran-asr` still training, `tadabur-Whisper-Large` still "coming soon" (Day 10+), `sherpa-onnx` stable at 1.13.3, `livekit_client` v2.9.0 still in dev prerelease.

---

## 2026-06-25 Night (03:33 UTC) — arXiv:2504.12254 (15,000h Weakly Supervised Arabic Conformer, SOTA Claims) + IqraAI MIT Multilingual Recitation Checker + Tarteel 7-Day Stall (Round 40)
*Full report: [research/2026-06-25/night-0333.md](2026-06-25/night-0333.md)*

### Insight 1 — arXiv:2504.12254 (April 2026): 15,000-Hour Weakly Supervised Conformer for Arabic ASR, Claims SOTA — 40-Round Blind Spot
"Advancing Arabic Speech Recognition Through Large-Scale Weakly Supervised Learning" (Salhab, Elghitany, Sait et al., April 2026, "Work in Progress"). Conformer trained **from scratch** on **15,000 hours** of weakly annotated Arabic speech (MSA + Dialectal), using pseudo-labels from a base model — **no manual transcriptions**. Largest Arabic ASR training run found across 40 rounds (vs. Tadabur 1,400h, muaalem 848h, Tarteel ~1,250h). Claims SOTA surpassing open and closed-source models on standard Arabic benchmarks. **Why it matters:** (1) A 15,000h Conformer backbone, fine-tuned on Quran, would dominate — more acoustic headstart than Whisper-large-v3's multilingual pretraining. (2) The weakly supervised recipe is Sakina's data flywheel: every recorded teacher-student session can train a better Arabic ASR backend without manual transcription. (3) Conformer = streaming-compatible + ONNX-exportable (same family as muaalem-v3, NVIDIA pcd). **Critical unknowns:** Weights NOT confirmed released ("Work in Progress"). License, HuggingFace handle, benchmark details (which corpora?) all unconfirmed. **Sakina actions: (1) Search HuggingFace for `mahmoudsalhab`/`elghitany`/`sait` — if weights exist, benchmark on `Buraaq/quran-audio-text-dataset`; (2) If unreleased, apply the recipe to Sakina's session audio at 100h+ — this converts recorded live sessions into ASR training data at scale; (3) Add to pitch: "Every Sakina session trains a better AI engine without manual labels — a data flywheel no AI-only app can build."**

### Insight 2 — `AbdirahmanNomad/IqraAI` (MIT, GitHub): First Open-Source Multilingual Quran Recitation Checker (Somali + Amharic + Swahili) — Uncontested East African Market Vertical
`github.com/AbdirahmanNomad/IqraAI` (MIT, Python, 5 commits). Recitation analysis pipeline on `tarteel-ai/whisper-base-ar-quran`: per-word color-coded diff (green/red/yellow), Iqra mode (select-verse → record → diff), Hijaiyah letter practice, export as **SRT subtitle file**. Multilingual translations: Arabic, English, **Somali, Amharic, Swahili** — first Quran recitation tool covering East African languages in 40 rounds. ~500MB model download on first run. **Strategic signals:** (1) Somali-speaking (~30M) and Amharic-speaking Ethiopian (~30M) Muslim communities are completely uncovered by every competitor — Sakina should recruit Somali/Ethiopian certified teachers as a distinct market wedge. (2) SRT export = timestamped word-accuracy log overlayable on a recording — Sakina should add this to teacher review mode: teacher sees a color-coded subtitle track during replay of the student session. (3) The Python verse-matching loop is a readable reference implementation (Tarteel Whisper → transcript → diff vs. expected text). **Limitations:** batch-only (no live streaming), word-correct/incorrect only (no tajweed rule ID), early-stage (5 commits). **Sakina actions: (1) Clone and study the verse-matching loop for the Dart/Flutter port; (2) Adopt SRT-style per-word accuracy overlay in teacher review screen; (3) Open a Somali/Amharic teacher recruitment channel — first mover in an uncontested vertical.**

### Insight 3 — Tarteel 7-Day Stall Confirmed (v5.78.2 June 19 → No v5.79+ as of June 25) — Longest Release Gap in 40 Rounds; `livekit_client` v2.9.0-dev.0 Prerelease Available
**Tarteel:** v5.78.2 (June 19) remains the latest stable across all sources — **7 days without a release**, the longest gap observed in 40 rounds of monitoring. No Makharij announcement. No new blog posts (most recent: Feb 2026). X/Twitter shows no June feature announcement. Interpretation: either preparing a larger bundled release (Makharij beta + Mushaf redesign) or the hotfix sprint simply ended. Sakina's Q3 2026 ship window remains fully open. **`livekit_client` v2.9.0-dev.0** (pub.dev, June 21, 2026): first prerelease of next major version signals a feature branch forming. v2.8.1 stable added "agent deployment targeting" — Sakina should use this to route AI pre-session warmup (Groq Orpheus TTS + KheemP scoring) to a separate LiveKit Agent node from the human teacher WebRTC session. Wait for v2.9.0 stable before locking in AI-agent integration pattern. Other monitors: `uzair0/quran-asr` still training (daily checkpoint saves), `tadabur-Whisper-Large/Medium` still "coming soon" (10 days since Small released), `sherpa-onnx` still 1.13.3.
...
### Insight 1 — IQRA 2026 Interspeech Challenge Finalized: Recognition Is Solved (0.16% PER) But Mispronunciation JUDGMENT Peaks at ~72% F1 With Only ~30% Baseline Precision — The Moat, Quantified
arXiv:2603.29087 **v2** (finalized; Round 33 had it only as a baseline reference). **19 teams** (nearly 2×). Best system: **+0.2787 absolute F1** over baseline → SOTA Arabic MDD F1 **≈72%** (consistent with 2511.17477's 70.53%). Organizer baselines on **`QuranMB.v2`**: Baseline-1 F1 44.14% (recall 77.07%, **precision 30.93%**); Baseline-2 F1 40.42% (precision 27.15%). **What won: "real mispronunciation data + careful data-quality control" — not architecture** (hard-confirms the Round 32 BAIC insight); plus the **first generative LALMs** applied to Arabic MDD. New substrate: **`Iqra_Extra_IS26`** (first authentic human mispronounced MSA corpus), `QuranMB.v2`, `Iqra_TTS` 52h; live HF leaderboard/submission Spaces. **Strategic punchline:** phoneme *recognition* is solved (0.16% PER, R30) but *judging whether a learner erred* is unreliable — esp. precision (~2 of 3 flags are false alarms at baseline). This converts "AI can't replace teachers" from peer-reviewed opinion into a measured accuracy ceiling, and Tarteel's own June-2026 reviews show the failure live ("re-flags words it previously marked correct"). **Sakina action:** benchmark the scorer on `QuranMB.v2`+`Iqra_Extra_IS26`; lead the pitch with the precision number; design live-session UI to harvest teacher-labeled real errors (the #1 data lever, uncollectable by AI-only apps).

### Insight 2 — `Harf-Speech` (arXiv:2604.06191, Mar 2026): Clinically-Validated Modular Arabic Phoneme Scorer = Sakina's Exact Blueprint, With Model (OmniASR, 8.92% PER) and Pearson-0.791 Eval Target
Harf-Speech scores Arabic pronunciation at phoneme level on a clinical scale via a **modular, interpretable pipeline that matches Sakina's Round 31/32 design 1:1**: MSA phonetizer (G2P) → fine-tuned speech-to-phoneme → **Levenshtein alignment** → **blended LCS + edit-distance scorer**. Best model **`OmniASR-CTC-1B-v2` @ 8.92% PER**. Clinically validated by **3 certified SLPs on 40 utterances: Pearson r=0.791, ICC(2,1)=0.659**, outperforming end-to-end frameworks. **Sakina action:** clone this as the scorer reference implementation; eval with Pearson/ICC vs. a Sakina teacher panel (more meaningful than F1 for a learning app — target r>0.79).

### Insight 3 — `facebook/omniASR-CTC-1B` (Apache-2.0): Meta Omnilingual ASR = New Apache Arabic-Phoneme Backbone for Accent Robustness; Monitors Stable (sherpa 1.13.3, Tarteel v5.78.2)
`huggingface.co/facebook/omniASR-CTC-1B` — wav2vec2+CTC, **1,600+ languages incl. Arabic**, **Apache-2.0** (no NC clause). Harf-Speech proves an 8.92%-PER Arabic-phoneme fine-tune. Gives Sakina a **third** Apache/MIT scorer base, ranked: `obadx/muaalem-v3_2` (0.16% PER, Quran-native, primary) → `omniASR-CTC-1B` (broad pretraining → better for non-native L2 learner accents: Turkish/Urdu/Indonesian diaspora) → Hafs2Vec recipe. **Monitors:** `sherpa_onnx` still **1.13.3** (no change); Tarteel still **v5.78.2** (June 19), **no v5.79+** (~5-day stall, no Makharij announcement); `uzair0/quran-asr` still training (HF policy-blocked this session, trigger not met). **Env note:** arxiv.org + huggingface.co both returned CONNECT-403 egress blocks this session — facts sourced via WebSearch snippets; a future run on a less-restricted network should directly fetch 2603.29087v2 + 2604.06191 for full tables.

---

## 2026-06-24 Daytime (07:16 UTC) — NVIDIA FastConformer `pcd` (CC-BY-4.0, EveryAyah-trained, Diacritized Output) + QuranCLAP v2 Verse-ID at 87% Recall — Both Absent From 32 Prior Rounds (Round 33)
*Full report: [research/2026-06-24/daytime-0716.md](2026-06-24/daytime-0716.md)*

### Insight 1 — `nvidia/stt_ar_fastconformer_hybrid_large_pcd_v1.0` (July 2025, CC-BY-4.0): First Open Classical Arabic ASR With Diacritized Output — Trained on Tarteel EveryAyah, SOTA — Never Covered in 32 Prior Rounds
arXiv:2507.13977 (July 18, 2025, IEEE). **`nvidia/stt_ar_fastconformer_hybrid_large_pcd_v1.0`** — FastConformer Hybrid Large (~115M params, Transducer+CTC), the first unified public model for **both MSA and Classical Arabic** with diacritized output. "pcd" = Punctuation + Character + **Diacritics** → outputs full tashkeel from speech. Training data: FLEURS + **Tarteel EveryAyah** (Quran-specific) + Common Voice Arabic. **License: CC-BY-4.0** — commercial use permitted with attribution; more permissive than `FaisaI/tadabur-Whisper-Small` (CC BY-NC blocks commercial). Paper claims SOTA accuracy with diacritics for Classical Arabic. Referenced in IQRA 2026 (arXiv:2603.29087) as a baseline. Companion without diacritics: `nvidia/stt_ar_fastconformer_hybrid_large_pc_v1.0`. **Sakina action (urgent):** Benchmark `pcd` model on `Buraaq/quran-audio-text-dataset` — if WER < 8%, this becomes the primary diacritized cloud-ASR option. The diacritized output enables the complete tajweed diff pipeline immediately (`pcd output → char-diff vs. expected text → tajweed rule`), and CC-BY-4.0 eliminates the NC licensing friction of tadabur-Whisper-Small. Exact WER not yet retrieved (HuggingFace 403); check `nvidia/stt_ar_fastconformer_hybrid_large_pcd_v1.0/discussions/2` for the SOTA comparison thread.

### Insight 2 — `yazinsai/offline-tarteel` (2025–2026, Open): 87% Recall Verse-ID From Audio via FastConformer + QuranCLAP v2 FAISS Targeting 95% — Audio-Embedding Alternative to Text-Based Tracking
`github.com/yazinsai/offline-tarteel` identifies surah+ayah from 3–5s audio without requiring a transcript. Best current: **NVIDIA FastConformer, 87% recall, 115 MB, 0.33s latency** (targeting 95% — gap still open). Two experimental architectures in `RESEARCH-audio-to-verse.md`: (1) **QuranCLAP v2** (CLIP-style contrastive): audio → 256-dim speaker-invariant embedding → FAISS NN search over all 6,236 verses — single forward pass + NN lookup, no ASR needed; (2) HuBERT/WavLM embedding + FAISS. **Distinct from `yayaiu6` tracker** (Round 31): `yayaiu6` does text-based Levenshtein+NW tracking of recitation position; `offline-tarteel` does audio-embedding-based verse identification — complementary. **Sakina action:** Clone + read `RESEARCH-audio-to-verse.md`; consider QuranCLAP v2 as a fallback verse-lock for learners with poor transcription quality (non-native accent, heavy tajweed errors). Also found: `Habib-HF/tarbiyah-ai-whisper-medium-merged` (MIT) = Whisper-medium merged (MSA fine-tune + Qira'at fine-tune) for "Tilawah AI" at tilawa.hhammad.com — fourth distinct Tilawa-named competitor, personal-project tier, no WER published. Tarteel still at v5.78.2 (June 19), no v5.79+ found (5 days stalled). `uzair0/quran-asr` still training (daily commits, monitor trigger not met).
...
### Insight 1 — `yayaiu6/Real-Time-Quran-recitation-tracker-System` (MIT, Nov 2025–Apr 2026, 121★): An Open-Source LIVE Follow-Along Engine That Documents the Exact Algorithm — No Longer a Build-From-Scratch Problem
The single biggest blind-spot across 30 rounds — *how to actually build real-time recitation position tracking* — is now resolved by a public MIT repo: `github.com/yayaiu6/Real-Time-Quran-recitation-tracker-System` (created 2025-11-21, last push 2026-04-26). It is "karaoke for Quran" with a fully documented live pipeline: selectable ASR (Groq Whisper cloud OR NVIDIA NeMo Conformer-CTC local, recommends `MostafaAhmed98/Conformer-CTC-Arabic-ASR`) → **text-based fuzzy alignment, NOT DTW**: (1) candidate Quranic segments ranked by normalized Levenshtein + length-mismatch penalty, (2) **Needleman-Wunsch global alignment** maps spoken→mushaf words. Dual-mode: "tracking" searches a **60-ahead/15-behind word sliding window** for continuous recitation; "search" mode scans the whole page after consecutive low-confidence chunks. Cumulative 6–10s audio buffer for context; Socket.IO streams word results + skip/mismatch alerts; skipped-verse detection at ≥12-word gap. Tunables in `backend/config.py` (word-sim 0.45, segment 0.5, window 60). **This confirms Tarteel's position-tracking has no algorithmic secret sauce** — the moat is data + latency engineering, not the matching logic. **Sakina action:** Read the README + config.py directly; port the matching loop to Dart/Flutter on top of `record` 7.1.0 + `sherpa_onnx` 1.13.3. Companion references: `sayedmahmoud266/quran-ai-transcriping` (RapidFuzz + PyQuran constraint propagation, batch-only), `lafzize` (CTC forced-alignment for playback timestamps), Qurani.ai QRC WebSocket API (buy-vs-build, returns word-index position data).

### Insight 2 — `quranic-phonemizer` v2.7 (PyPI, June 18 2026, MIT): Tajweed-Aware Quran G2P With Built-In Fuzzy `ref_text` Matching — Solves Phonetic Matching AND Partial-Position Lock in One Package
`pip install quranic-phonemizer` (v2.7, June 18 2026, MIT, Py 3.8–3.13). Tajweed-aware G2P for Hafs ʿan ʿAsim with a 69–71 phoneme inventory (IPA + custom Tajweed phonemes: Idgham, Iqlab, Ikhfaa, Qalqala, Tafkheem, Waqf). Critically, its API accepts raw `ref_text` **with built-in fuzzy matching** that maps noisy/partial recited text onto the correct ayah and returns position — directly serving both the follow-along position problem and the mispronunciation-detection problem. NeurIPS 2025 Muslims-in-ML paper; companion dataset `hetchyy/everyayah-phonemes`. **Full supporting phonetic stack confirmed 2025–2026, all open:** `camel-tools` 1.6.0 (Jun 8 2026, MIT — normalization+diacritization), `rapidfuzz` 3.14.5 (Arabic-diacritic-correct surface fuzzy), `panphon` 0.22.2 (Jun 12 2025, MIT — weighted articulatory-feature phoneme edit distance), confusion-matrix substitution costs from El Kheir (Interspeech 2025) and the Jan-2025 Springer MDD paper, AraS2P (arXiv 2509.23504, #1 at IqraEval) as the speech→phoneme front-end. **Negative finding:** no maintained Arabic Soundex/Metaphone package exists in 2025–2026 — superseded by IPA-G2P + phoneme edit distance. **Sakina action:** Use `quranic-phonemizer` `ref_text` mode as both the ayah-lock and the Tajweed-IPA reference; score mispronunciations via weighted Needleman-Wunsch with panphon/El-Kheir confusion costs.

### Insight 3 — Tarteel Production Stack Fully Mapped: NVIDIA NeMo+Riva+Triton (~4% WER, <200ms), React Native, NO Public ASR API — Sakina Cannot Piggyback, Must Self-Host
Confirmed via NVIDIA case study + CoreWeave/Zeet case studies (2025–2026): Tarteel's production ASR is **NVIDIA NeMo (train) → Riva (serve) → Triton (multi-GPU)**, Conformer/FastConformer family, ~4% WER on Quranic Arabic, <200ms streaming latency, trained on >75,000 min (~1,250h) curated recitation. Their public HuggingFace `whisper-base-ar-quran` (5.75% WER) models are **research artifacts, not production**. GitHub fork pattern confirms the stack: `nemo2riva` (Feb 2026), `celery-exporter` (Apr 2026, Celery+Prometheus), `fastlane` (Jan 2026). **No public ASR/inference API** — officially confirmed (support article Sept 2025); they redirect devs to QUL, which is static Quran *data* (JSON/SQLite, Rails+PostgreSQL, MIT, 894★), not an API. Mobile = **React Native** (`com.mmmoussa.iqra`, v5.76.9, ~14M downloads) — NOT Flutter. Infra migrated AWS→CoreWeave (~22% latency↓, ~56% cost↓). `tarteel-ml` archived since Dec 2021 (DeepSpeech-era, irrelevant). Dataset `tarteel-ai/everyayah` (~117GB, professional reciters) likely CC BY-NC-ND 4.0 — verify before any use. **Sakina takeaways:** (1) Cannot call Tarteel's ASR — self-host via sherpa/whisper.cpp or use Groq/Qurani.ai. (2) Their tracking algorithm is the same openly-documented streaming-ASR-vs-text alignment (yayaiu6). (3) They're React Native; Sakina's Flutter + live-teacher angle stays differentiated.

### Insight 4 — Flutter Real-Time Audio: Capture-Once-Fork Architecture Required (iOS AVAudioSession Conflict Confirmed) — record 7.1.0 + audio_session + sherpa_onnx 1.13.3
Confirmed current versions (all 2026): `record` 7.1.0 (Jun 11, BSD-3, `startStream` PCM16 broadcast), `sherpa_onnx` 1.13.3 (Jun 16, Apache-2.0), `flutter_webrtc` 1.5.2 (Jun 20, MIT). **Critical:** running flutter_webrtc + a recorder as two independent mic openers is unreliable on iOS (one app-wide AVAudioSession; categories clobber — documented Feb 2026 LiveKit issue #996). **Required pattern: capture mic ONCE** (`record` broadcast or Picovoice `flutter_voice_processor` multi-listener) → coordinate with `audio_session` package → fork PCM16 to WebRTC + ASR; or tap WebRTC local AudioTrack (needs native glue). **AVOID `mic_stream`** (dormant since early-2024, GPL-3.0; prior "March 2026" date was a hallucination — debunked). For fine-tuned Arabic Whisper on-device, `whisper_ggml_plus` 1.5.2 (Apr 1 2026, MIT) is the smoother conversion path (HF→ggml first-class) vs sherpa's fixed-model-name export limitation. **Net: every layer of the Sakina pipeline now has a confirmed 2025–2026 open/MIT/Apache implementation — the two biggest unknowns (follow-along + phonetic matching) are both buildable from MIT reference code.**
...
### Insight 1 — Groq Orpheus Arabic Saudi TTS (Live Since Jan 13, 2026): Abdullah & Aisha Voices, Sub-200ms, $40/1M Chars — Never Covered in 24 Prior Rounds
`canopylabs/orpheus-arabic-saudi` on GroqCloud. **Two Saudi dialect voices: Abdullah** (male, professional/calm, assistants/enterprise) and **Aisha** (female, professional/clear, customer support). Sub-200ms time-to-first-byte, ~100 chars/second streaming, $40/1M chars. Cost floor for Sakina's AI feedback audio for the entire early beta: **under $0.02** (30 chars/feedback × 10,000 messages = $0.012). Prior 24 rounds covered Groq exclusively for STT (Whisper Large v3 Turbo $0.04/hr) — TTS was a blind spot. **Complete pipeline now on Groq:** `mic → Groq Whisper STT → tajweed error detection → Groq Orpheus TTS → spoken Arabic feedback`. Same API key, just change endpoint to `POST /openai/v1/audio/speech`, `model: canopylabs/orpheus-arabic-saudi`, `voice: aisha`. **Sakina action:** Add Orpheus TTS to the AI tutor mode (pre-session warmup before live teacher joins) and error feedback loop (mispronounced word → AI demonstrates correct pronunciation in Aisha's voice). Saudi dialect is culturally appropriate for GCC core market; test with Levantine/Egyptian users for accent acceptability before shipping globally.

### Insight 2 — Tadabur Whisper Fine-tuned Models on HuggingFace (FaisaI org, April 2026) — First Whisper Variants Domain-Adapted for Full 1,400h Quranic Diversity
**FaisaI/tadabur** (HuggingFace) includes Whisper models fine-tuned specifically for Quranic ASR alongside the 1,400h Tadabur dataset. Domain adaptations not found in any prior model: prolonged Madd phoneme durations (2–6× standard vowel lengths), full tajweed acoustic variation (ikhfa nasalization, qalqalah echo, ghunnah), melodic mujawwad recitation style, 600+ distinct reciters. Prior 24 rounds repeatedly cited the Tadabur *dataset* (arXiv:2604.18932) — these *model weights* were never surfaced. Expected WER improvement over KheemP/whisper-base-quran-lora (5.98%, trained on ~390h EveryAyah only): potentially significant due to 3.6× more training data and mujawwad style coverage. **License:** CC BY-NC 4.0 — NC clause requires commercial license negotiation with author Faisal Alherran for production use, or use weights as training teacher for a commercial distilled model. **Sakina action:** Locate exact model card in FaisaI org → benchmark on 20 Quran-MD word clips → compare WER vs. KheemP (5.98%) and ICML 2026 QPS model → if <5%, adopt as primary ASR; if 5–8%, ensemble with Nemotron-3.5.

### Insight 3 — arXiv:2606.16629 Islamic LLMs Survey (June 15, 2026) + IslamicMMLU Leaderboard: Five Non-Negotiable Requirements for Sakina's AI Assistant Layer
"Islamic Large Language Models: From Knowledge Acquisition to Trustworthy and Hallucination-Resistant AI" (Mohammed Amine Mouhoub, June 15, 2026) is the most comprehensive Islamic AI safety framework yet published. Five requirements the survey identifies as non-negotiable: (1) curated source grounding, (2) retrieval+verification module, (3) citation-aware generation (every rule cites surah:ayah or hadith ref), (4) madhhab-aware plurality (Hanafi/Maliki/Shafi'i/Hanbali divergences must be represented, not collapsed), (5) human scholarly evaluation. **IslamicMMLU** (arXiv:2603.23750, March 2026): public LLM leaderboard, 10,013 MCQ (Quran: 2,013 Qs, Hadith: 4,000, Fiqh: 4,000). Gemini 3 Flash tops at 93.8% overall; Quran track best: 99.3%. Arabic-specific models significantly underperform frontier models. **Sakina action:** (1) Every AI-generated tajweed explanation must be retrieval-augmented over `TarteelAI/quranic-universal-library` (MIT, verified) — no bare LLM calls on Islamic content; (2) Run IslamicMMLU Quran track (2,013 Qs) on any LLM candidate for the Sakina AI assistant — minimum bar: >90%; (3) Add madhhab selector to teacher profile so tajweed rulings can be presented in the user's preferred school context.
